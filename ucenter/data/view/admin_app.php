...
<div class="container">
	<? if($a == 'ls') { ?>
	<h3 class="marginbot">
		应用列表<a href="admin.php?m=app&a=add" class="sgbtn">添加新应用</a>
	</h3>
	<? if(!$status) { ?>
	...
	<div class="mainbox">
		...
		<form action="admin.php?m=app&a=ls" method="post">
			...
			<table class="datalist fixwidth" onmouseover="addMouseEvent(this);">
				<tr>
					<th nowrap="nowrap"><input type="checkbox" name="chkall"
						id="chkall" onclick="checkall('delete[]')" class="checkbox" /><label
				...
				<tr>
					<td width="50"><input type="checkbox" name="delete[]"
						value="<?=$app['appid']?>" class="checkbox" /></td>
					...
					<td width="90"><div id="status_<?=$app['appid']?>"></div>
						<script id="link_<?=$app['appid']?>"
							testlink="admin.php?m=app&a=ping&inajax=1&url=<? echo urlencode($app['url']);?>&ip=<? echo urlencode($app['ip']);?>&appid=<?=$app['appid']?>&random=<? echo rand()?>"></script>
						<script>apps[<?=$i?>] = '<?=$app['appid']?>';</script></td>
					<td width="40"><a
						href="admin.php?m=app&a=detail&appid=<?=$app['appid']?>">编辑</a></td>
				</tr>
				<? $i++?>
				<? } ?>
				<tr class="nobg">
					<td colspan="9"><input type="submit" value="提 交"
						class="btn" /></td>
				</tr>
			</table>
	...
	<? } elseif($a == 'add') { ?>
	<h3 class="marginbot">
		添加新应用<a href="admin.php?m=app&a=ls" class="sgbtn">返回应用列表</a>
	</h3>
	<div class="mainbox">
		<table class="opt">
			<tr>
			...
			<tr>
				<td><input type="radio" name="installtype" class="radio"
					checked="checked"
					onclick="$('url').style.display='';$('custom').style.display='none';" />URL 安装 (推荐) <input type="radio" name="installtype"
					class="radio"
					onclick="$('url').style.display='none';$('custom').style.display='';" />自定义安装</td>
			</tr>
		</table>
		<div id="url">
			<form method="post" action="" target="_blank"
				onsubmit="document.appform.action=document.appform.appurl.value;"
				name="appform">
				<table class="opt">
					<tr>
						<th>应用程序安装地址:</th>
					...
					<tr>
						<td><input type="text" name="appurl" size="50"
							value="http://domainname/install/index.php" style="width: 300px;" /></td>
					</tr>
				</table>
				<div class="opt">
					<input type="hidden" name="ucapi" value="<?=UC_API?>" /> <input
						type="hidden" name="ucfounderpw" value="<?=$md5ucfounderpw?>" /> <input
						type="submit" name="installsubmit"
						value=" 安 装 " class="btn" />
				</div>
			</form>
		</div>
		<div id="custom" style="display: none;">
			<form action="admin.php?m=app&a=add" method="post">
				...
				<table class="opt">
					...
					<tr>
						<td><input type="text" class="txt" name="apppath" value="" />
						</td>
					...
					<tr>
						<td><input type="text" class="txt" name="viewprourl" value="" />
						</td>
					...
					<tr>
						<td><input type="text" class="txt" name="apifilename"
							value="uc.php" /></td>
					...
					<tr>
						<td><input type="radio" class="radio" id="yes"
							name="synlogin" value="1" /><label for="yes">是</label>
							<input type="radio" class="radio" id="no" name="synlogin"
							value="0" checked="checked" /><label for="no">否</label>
						</td>
					...
					<tr>
						<td><input type="radio" class="radio" id="yes"
							name="recvnote" value="1" /><label for="yes">是</label> <input
							type="radio" class="radio" id="no" name="recvnote" value="0"
							checked="checked" /><label for="no">否</label></td>
						<td></td>
					</tr>
				</table>
				<div class="opt">
	...
	<? } else { ?>
	<h3 class="marginbot">
		编辑应用<a href="admin.php?m=app&a=ls" class="sgbtn">返回应用列表</a>
	</h3>
	<? if($updated) { ?>
	<div class="correctmsg">
		<p>更新成功。</p>
	...
	<div class="mainbox">
		<form action="admin.php?m=app&a=detail&appid=<?=$appid?>" method="post">
			...
			<table class="opt">
				...
				<tr>
					<td><input type="text" class="txt" name="authkey"
						value="<?=$authkey?>" /></td>
					<td>只允许使用英文字母及数字，限 64 字节。应用端的通信密钥必须与此设置保持一致，否则该应用将无法与 UCenter 正常通信。</td>
				</tr>
				<tr>
				...
				<tr>
					<td><input type="text" class="txt" name="apppath"
						value="<?=$apppath?>" /></td>
					<td>默认请留空，如果填写的为相对路径（相对于UC），程序会自动转换为绝对路径，如 ../</td>
				</tr>
				<tr>
				...
				<tr>
					<td><input type="text" class="txt" name="viewprourl"
						value="<?=$viewprourl?>" /></td>
					<td>URL中域名后面的部分，如：/space.php?uid=%s 这里的 %s 代表uid</td>
				</tr>
				<tr>
				...
				<tr>
					<td><input type="radio" class="radio" id="yes" name="synlogin"
						value="1" <?=$synlogin[1]?> /><label for="yes">是</label> <input
						type="radio" class="radio" id="no" name="synlogin" value="0"
						<?=$synlogin[0]?> /><label for="no">否</label></td>
					<td>开启同步登录后，当用户在登录其他应用时，同时也会登录该应用。</td>
				</tr>
				<tr>
					<th colspan="2">是否接受通知:</th>
				...
				<tr>
					<td><input type="radio" class="radio" id="yes" name="recvnote"
						value="1" <?=$recvnotechecked[1]?> /><label for="yes">是</label> <input type="radio" class="radio" id="no" name="recvnote"
						value="0" <?=$recvnotechecked[0]?> /><label for="no">否</label>
					</td>
					<td></td>
				</tr>
			</table>
			...
			<table class="opt">
				...
				<tr>
					<th><textarea class="area" onFocus="this.select()">
