...
<p>
	<a href="<?php echo $PHP_SELF;?>?action=db">如果您已确认完成上面的步骤,请点这里升级</a>
</p>
...
<?php
    ...
    while ($app = $db->fetch_array($query)) {
        if (authcode($app['authkey'], 'DECODE', UC_MYKEY))
            continue;
...
function createtable($sql, $dbcharset)
{
    ...
    return preg_replace("/^\s*(CREATE TABLE\s+.+\s+\(.+?\)).*$/isU", "\\1", $sql) . (mysql_get_server_info() > '4.1' ? " ENGINE=$type default CHARSET=" . UC_DBCHARSET : " TYPE=$type");
}
