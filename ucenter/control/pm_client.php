...
class pm_clientcontrol extends base
{
    ...
    function onsend()
    {
        ...
        if (! $this->submitcheck()) {
            ...
            if ($_POST['saveoutbox']) {
                ...
                if ($sent) {
                    $this->message('pm_send_succeed', 'index.php?m=pm_client&a=ls&folder=' . ($_POST['folder'] ? $_POST['folder'] : 'inbox'), 1, array(
                        '$sent' => $sent
                    ));
                } else {
    ...
    function ondelete()
    {
        $this->_auth();
        $uid = $this->user['uid'];
        $touids = ! empty($_GET['delete']) ? array(
            $_ENV['pm']->pmintval($_GET['delete'])
        ) : (! empty($_GET['pmid']) ? array(
            $_ENV['pm']->pmintval($_GET['pmid'])
        ) : @$_POST['delete']);
